...
class TradingAgentsGraph:
    ...
    def propagate_market(
        ...
        trader_prompt = (
            ...
            f"BUY_YES. Confidence below 0.85 will be filtered out by the routine "
            f"anyway — high confidence means high evidence, not high narrative.\n\n"
    ...
    def propagate_stock(
        ...
        trader_prompt = (
            ...
            f"Confidence calibration — use the full scale:\n"
            f"  0.48-0.52: genuinely 50/50, bull and bear cases cancel out → HOLD\n"
            f"  0.53-0.59: slight edge, one side has marginally better evidence → HOLD or weak directional\n"
            f"  0.60-0.69: clear edge, concrete recent catalyst on one side → LONG or SHORT\n"
            f"  0.70+:     high conviction, multiple concrete catalysts aligning → LONG or SHORT\n"
            f"Do NOT anchor to 0.52 for all uncertain cases. If one side is even slightly stronger, "
            f"express that in a confidence between 0.53-0.59. Reserve 0.48-0.52 for true coin flips.\n\n"
