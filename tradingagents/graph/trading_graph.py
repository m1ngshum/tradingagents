...
class TradingAgentsGraph:
    ...
    def propagate_market(
        ...
        trader_prompt = (
            ...
            f"DEFAULT TO HOLD (important): Historical calibration on this bot shows "
            f"BUY_YES wins ~10% of the time and BUY_NO wins ~71% of the time. The bull "
            f"researcher is built to find a YES case and routinely over-weights weak "
            f"evidence. Treat HOLD as the default direction unless you have (a) a "
            f"specific recent catalyst, (b) base-rate support for your direction, AND "
            f"(c) clear pricing dislocation. If the bull case is mainly 'narrative + "
            f"news momentum' without a concrete catalyst, choose HOLD or BUY_NO, not "
            f"BUY_YES. Report honest confidence — high confidence means high "
            f"evidence, not high narrative.\n\n"
            ...
            f"phrase, or statement (e.g. 'Will X say Y by date Z?'), default to "
            f"HOLD or BUY_NO. Historical calibration: this bot lost 7-of-7 on the "
            f"2026-05-14 Trump-Xi quote cluster by leaning BUY_YES on 'common word' "
            f"reasoning. The bull researcher's 'speaker uses this word regularly' "
            f"signal is consistently weaker than the market's price discount on "
            f"specific-event-window utterance probability. Only lean BUY_YES when "
            f"(a) the question uses 'again' AND the speaker said it in the same "
            f"event series, OR (b) you have a recent verbatim quote of the speaker "
            f"using that exact phrase in the same context window. The market price "
            f"is your prior — do not move away from it on general-frequency reasoning.\n\n"
            f"ELECTORAL MARKETS (important): If the market question asks whether a "
            f"specific political party or candidate will win the most seats, win an "
            f"election, or win a majority (e.g. 'Will Party X win the most seats in "
            f"Election Y?'), apply strong base-rate skepticism toward insurgent or "
            f"challenger parties. In first-past-the-post electoral systems, incumbent "
            f"parties and established mainstream parties win outright the large majority "
            f"of the time (historically 70-85%+). Insurgent parties — even those with "
            f"high poll ratings, strong media narratives, and compelling momentum — "
            f"rarely win outright in their first or second electoral cycle. Do NOT let "
            f"the bull researcher's narrative evidence (polling leads, rally attendance, "
            f"media momentum) override this base rate unless you have very recent, "
            f"concrete, high-quality polling data showing a clear multi-point lead in "
            f"the specific constituency or election in question. When in doubt, HOLD "
            f"rather than BUY_YES on an insurgent win.\n\n"
            f"POPULARITY CONTESTS (important): If the market question asks whether a "
            f"specific contestant, performer, or participant will win a talent show, "
            f"reality competition, or public vote (e.g. 'Will X win American Idol?', "
            f"'Will Y win Big Brother?'), apply strong base-rate skepticism. In any "
            f"multi-contestant elimination competition, each individual contestant has "
            f"a low prior probability of winning (1/N where N is remaining contestants). "
            f"Social media buzz, strong performances, and fan momentum are routinely "
            f"cited for contestants who do NOT win — these signals are weak predictors. "
            f"Do NOT BUY_YES on a specific contestant unless they have overwhelming, "
            f"concrete evidence of dominance (e.g. near-certain favourite in betting "
            f"markets at >70% implied probability). When in doubt, HOLD rather than "
            f"BUY_YES on any individual contestant.\n\n"
    ...
    def propagate_stock(
        ...
        trader_prompt = (
            ...
            f"BULL CASE:\n{bull_arg}\n\n"
            f"BEAR CASE:\n{bear_arg}\n\n"
            f"HOLD WHEN GENUINELY BALANCED (important): The base rate of correctly "
            f"predicting 5-day direction on individual stocks is ~50%, so HOLD is the "
            f"right call when bull and bear cases truly cancel. But \"balanced\" is a "
            f"high bar, not a fallback. If you find yourself describing the bull case as "
            f"\"real positives\" AND the bear case as \"real risks\" but still landing on "
            f"HOLD, ask: which side has the better risk/reward over the next 5 days? "
            f"Often one side does — take that side. Do NOT call something \"balanced\" "
            f"just because both sides have arguments.\n\n"
            f"PRE-EVENT SETUPS — TAKE THE ASYMMETRIC SIDE: When a binary event (earnings, "
            f"announcement, decision) is imminent, the trade is not automatically HOLD. "
            f"Ask: how is the market positioned? Are expectations elevated or compressed? "
            f"Has analyst sentiment recently shifted? Pre-event setups often have clear "
            f"asymmetric risk/reward — go LONG when expectations are low AND recent news "
            f"flow is positive; go SHORT when expectations are stretched AND recent news "
            f"flow is negative; HOLD only when both sides are genuinely mixed.\n\n"
            f"CONFIDENCE — REPORT HONEST EVIDENCE STRENGTH: Confidence is the probability "
            f"your recommendation is correct, 0.0 to 1.0. High confidence requires high "
            f"evidence, not high narrative. Vary the number based on how strong the "
            f"asymmetry is — a clear setup with multiple aligned signals warrants 0.65+, "
            f"a thin asymmetry warrants 0.55-0.65, true balance warrants HOLD at 0.50-0.55. "
            f"Do not pick a number from a band; pick the number that reflects the actual "
            f"evidence strength.\n\n"
            f"SHORT — REQUIRES NEGATIVE CATALYST OR STRETCHED SETUP: Single-name 5-day "
            f"shorts have a poor base rate (mildly positive drift, squeeze risk, "
            f"asymmetric loss). Take SHORT when EITHER (a) a specific negative catalyst "
            f"has materialized (guidance cut, fraud allegation, sector breakdown with "
            f"this name as a clear loser), OR (b) the stock is stretched into a "
            f"high-uncertainty event with recent news flow turning negative (analyst "
            f"downgrades, price targets cut, options skew). \"Overvalued\" or \"momentum "
            f"fading\" alone is still insufficient.\n\n"
