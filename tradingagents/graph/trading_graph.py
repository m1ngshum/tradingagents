...
class TradingAgentsGraph:
    ...
    def propagate_stock(
        ...
        trader_prompt = (
            ...
            f"BULL CASE:\n{bull_arg}\n\n"
            f"BEAR CASE:\n{bear_arg}\n\n"
            f"DEFAULT TO HOLD (important): The base rate of correctly predicting 5-day "
            f"direction on individual stocks is roughly 50%. The bull and bear researchers "
            f"are built to argue their respective cases and routinely over-weight weak "
            f"evidence. Treat HOLD as the default unless you have (a) a specific recent "
            f"catalyst (earnings beat/miss, guidance change, regulatory event, sector "
            f"rotation with named driver), (b) base-rate support for your direction, AND "
            f"(c) clear pricing dislocation relative to the catalyst. If the case is mainly "
            f"'narrative + technicals' without a concrete catalyst, choose HOLD.\n\n"
            ...
            f"evidence, not high narrative. Do NOT pick a number from a band — pick the "
            f"number that reflects how strongly the actual evidence supports your call. "
            f"If the bull and bear cases are roughly even, that IS HOLD with moderate "
            f"confidence; vary the number based on how cleanly they cancel out.\n\n"
            f"BASE-RATE SKEPTICISM FOR SHORTS: SHORTING individual large-cap equities has "
            f"a poor base rate over 5-day windows — historical drift is mildly positive, "
            f"single-name shorts get squeezed on minor good news, and the cost of being "
            f"wrong is asymmetric. Prefer HOLD over SHORT unless you have a specific "
            f"recent negative catalyst (guidance cut, fraud allegation, sector breakdown "
            f"with this name as a clear loser). Generalized 'overvalued' or 'momentum "
            f"fading' arguments are not enough.\n\n"
